...
describe('usertokens import', () => {
  it('imports NovaONE, NRW, and DBIS 138 ecosystem tokens', () => {
    ...
    expect(userTokensForChain(138).some((t) => t.symbol === 'ETH')).toBe(true)
    expect(userTokensForChain(138).some((t) => t.symbol === 'USDT')).toBe(true)
    expect(userTokensForChain(138).length).toBeGreaterThanOrEqual(15)
  })
