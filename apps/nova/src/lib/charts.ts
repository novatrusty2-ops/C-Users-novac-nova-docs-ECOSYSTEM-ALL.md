...
function quoteAssetForChain(chainId: number): string {
  if (chainId === 33001) return 'USDT'
  if (chainId === 138) return 'USDC'
...
/**
 * Production chart series for mesh + DBIS custody tokens.
 * Seeded from symbol + chain + live mid so curves are stable and track oracle/mesh price.
 */
export function buildTokenChart(
  ...
  // Stables stay tight to peg; majors have higher vol
  const volatility = isMeshStable(symbol)
    ? 0.0015
    : mid >= 1000
      ? 0.008
      : mid >= 1
...
/** Portfolio total sparkline from current USD + seeded history */
export function buildPortfolioChart(
  totalUsd: number,
  range: ChartRange = '1D',
): TokenChartSeries {
  return buildTokenChart(0, 'PORTFOLIO', range, Math.max(totalUsd, 0.01))
}
