...
export function TokenChart({
  ...
  const gradId = `fill-${symbol.replace(/[^a-zA-Z0-9]/g, '')}-${chainId}`
  ...
  return (
    <section className="space-y-3">
      ...
      <div className={`overflow-hidden rounded-xl bg-nova-surface ${compact ? 'px-1 py-1' : 'px-2 pt-3 pb-1'}`}>
        <svg
          viewBox={`0 0 ${width} ${height}`}
          className={`${compact ? 'h-20' : 'h-44'} w-full animate-fade-up`}
          role="img"
