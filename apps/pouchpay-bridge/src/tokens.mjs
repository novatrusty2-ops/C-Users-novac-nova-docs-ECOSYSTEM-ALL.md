...
export const TOKENS = {
  ...
  /** Native ETH alias → WETH wrap on Alltra. */
  ETH: {
    symbol: "ETH",
    address: WETH,
    decimals: 18,
  ...
  /** Native BNB alias → WBNB wrap on Alltra (external). */
  BNB: {
    symbol: "BNB",
    address: WBNB,
    decimals: 18,
  ...
  /** Native TRX alias → WTRX wrap on Alltra (external). */
  TRX: {
    symbol: "TRX",
    address: WTRX,
    decimals: 18,
...
/** Symbols that quote as ETH / BNB / TRX natives (wraps under the hood). */
export const NATIVE_ALIAS_SYMBOLS = ["ETH", "BNB", "TRX"];
...
export function resolveToken(symbolOrAddress) {
  ...
  if (/^0x[0-9a-fA-F]{40}$/.test(raw)) {
    ...
    // Prefer canonical wrap symbols over ETH/BNB/TRX aliases for address lookups.
...
/** All production swap symbols including ETH/BNB/TRX native aliases. */
export function allSwapSymbols() {
